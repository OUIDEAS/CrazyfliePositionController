...
class _ExtposThread(Thread):
    def __init__(self,socket,cf,*args):
        super(_ExtposThread, self).__init__(*args)
        self._socket = socket
        self._cf = cf
        print("Starting Extpos Server")
    def run(self):
        while True:
            ...
            #self._cf.loc.send_extpos([0,0,0])
...
class ZMQServer():
    ...
    def __init__(self, base_url):
        ...
        extpos_srv = self._bind_zmq_socket(zmq.PULL,"extpos",ZMQ_EXTPOS_PORT)
        ...
        self._extpos_thread = _ExtposThread(extpos_srv, self._cf)
        self._extpos_thread.start()
